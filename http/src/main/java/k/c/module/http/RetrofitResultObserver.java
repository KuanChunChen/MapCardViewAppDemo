...
//        ToastUtils.showShort("UnKnow error");
    ...
    public void onNext(Result<T> t) {
        ...
        if (t.value == null) {
            onFailure(new RetrofitResultException(RetrofitResultException.PARSE_ERROR, "Return null data from server"));
            return;
        }

//        Result result = (Result)t;

        if(Constants.Http.SUCCESS == t.result.code) {
            try {
                onSuccess(t.value);
            }
            catch (Exception e){
    ...
    protected void onResultCode(int resultCode, Result result){
        ...
        onFailure(new RetrofitResultException(RetrofitResultException.RESULT_CODE_ERROR, "Return error result from server"));
    }
    ...
    public abstract void onSuccess(Collection<T>  data);
